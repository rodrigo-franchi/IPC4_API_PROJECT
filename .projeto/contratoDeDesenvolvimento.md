...
Com minha experiência prévia desenvolvendo aplicações utilizando inteligência artificial, e inspirado pelas valiosas dicas recebidas nos primeiros módulos do curso de pós-graduação, optei por uma abordagem estruturada e disciplinada para este projeto de API CRUD de clientes.

Decidi iniciar definindo um contrato de desenvolvimento. Um contrato de desenvolvimento é um acordo formal entre as partes envolvidas (desenvolvedor, cliente ou equipe) que estabelece claramente os requisitos funcionais, escopo do projeto, regras de validação, prazos e expectativas, servindo como base sólida para evitar mal-entendidos e garantir alinhamento desde o início.

Neste projeto, o contrato tomou a forma de cenários em Gherkin, utilizando Behavior Driven Development (BDD). Esses cenários descrevem o comportamento esperado da API de forma compreensível, como no exemplo abaixo:
...
## 📚 Documento de Contrato de Desenvolvimento
